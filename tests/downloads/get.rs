...
#[tokio::test]
async fn should_get_downloads() {
    mocked(|mock, debrid| async move {
        let r =
            resource::<Vec<models::downloads::Download>>("tests/.resources/downloads/index.json");
        ...
        let res = debrid.downloads().get().page(1).limit(25).send().await;
...
#[tokio::test]
async fn should_fail_to_get_downloads() {
    mocked(|mock, debrid| async move {
        Mock::given(matchers::method("GET"))
            .and(matchers::path("/downloads"))
            .respond_with(
