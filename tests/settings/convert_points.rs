...
#[tokio::test]
async fn should_convert_points() {
    mocked(|mock, debrid| async move {
        Mock::given(matchers::method("POST"))
            .and(matchers::path("/settings/convertPoints"))
            .respond_with(ResponseTemplate::new(204))
            .expect(1)
            .mount(&mock)
            .await;

        let res = debrid.settings().convert_points().send().await;
...
async fn should_fail_to_convert_points() {
    ...
    mocked(|mock, debrid| async move {
        Mock::given(matchers::method("POST"))
            .and(matchers::path("/settings/convertPoints"))
            .respond_with(
                ResponseTemplate::new(503).set_body_json(models::error::Error {
                    code: 25,
                    message: "service_unavailable".to_string(),
                }),
            )
            .expect(1)
            .mount(&mock)
        ...
        assert!(res.is_err_and(|e| matches!(e, Error::Debrid(DebridError::ServiceUnavailable))));
    })
    .await;
}
